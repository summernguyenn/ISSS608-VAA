...
### [**Practice: Compare Clustering-Based Correlation Visualizations using `corrplot` and `pheatmap`**]{style="color:#2E86C1; background-color: #FFFF99;"}

#### pheatmap - A matrix-style heatmap showing correlation values, with dendrograms on both axes for hierarchical clustering.
```{r}
library(pheatmap)
cor_matrix <- cor(wine[, 2:12])
pheatmap(cor_matrix, 
...
#### corrplot -  An ellipse-based correlogram with hierarchical clustering and black boxes highlighting detected variable clusters.
```{r}
...
| Feature                          | `pheatmap()`                                                                  | `corrplot()`                                                                     |
|----------------------------------|-------------------------------------------------------------------------------|----------------------------------------------------------------------------------|
| **Visual Style**                 | Heatmap with numerical values                                                 | Ellipse-shaped correlogram                                                       |
| **Clustering Display**           | Dendrograms along axes                                                        | Black boxes around clustered variables                                           |
| **Color Scheme**                 | Blue-white-red gradient for correlation coefficients                         | Blue-red scale with ellipse tilt and shape indicating correlation direction      |
| **Correlation Values**           | Displayed as exact numbers in cells                                          | Inferred from ellipse orientation and size                                       |
| **Best For**                     | Structural exploration, teaching, variable grouping clarity                  | Presentation-friendly, faster scanning, visual storytelling                      |
| **Cluster Interpretation**       | Clear visual linkage via tree structure                                       | Helpful rectangle groupings but no dendrograms shown                             |
| **Readability with Many Variables** | Moderate; can get cluttered                                                 | More compact and intuitive at a glance                                           |

In summary:

- `pheatmap()` allows for **precise value reading** and deeper exploration of the **cluster structure**, especially when teaching or analyzing.
- `corrplot()` is more **aesthetically engaging and compact**, making it ideal for presentations and quick insights.
