---
title: "Hands-on Exercise 8.1 - Choropleth Mapping in R"
author: "Nguyen Nguyen Ha (Summer)"
date: "June 12, 2025"
date-modified:  "June 12, 2025"
format:
  html:
    css: styles.css
    toc: true
...
Only the first 10 records are shown, as this is the default display of R to minimize resource usage and avoid overwhelming user. To specify the number of rows to be shown, *head()* function with `n` arguments can be used instead. For example, *head(mpsz, n = 15)* shows the first 15 rows of the `mpsz` data frame .
:::
...
### 3.4.2. Join the Attribute Data and Geospatial Data
...
# 4. Choropleth Mapping of Geospatial Data with **tmap**
...
## 4.1. Plot a Choropleth Map quickly using *qtm()*
...
## 4.2. Create a Choropleth Map using tmap’s elements
...
### 4.2.1. Drawing a Base map (*tm_shape()*)
...
### [**Practice: Prepare choropleth maps by using different classification methods supported by tmap and compare their differences**]{style="color:#2E86C1; background-color: #FFFF99;"}
...
### [**Practice:  Preparing choropleth maps by using similar classification method but with different numbers of classes (i.e. 2, 6, 10, 20). Compare the output maps.**]{style="color:#2E86C1; background-color: #FFFF99;"}
...
### 4.3.2. Custom break data classification
...
## 4.6 Draw Small Multiple Choropleth Maps
...
### 4.6.1. Assign multiple values to at least one aesthetic argument
...
### 4.6.2. Define a group-by variable in *tm_facets()*
...
## 4.7. Map Spatial Object Meeting a Selection Criterion
...
tm_shape(mpsz_pop2020[mpsz_pop2020$REGION_N == "CENTRAL REGION", ]) +
  tm_fill(
    ...
    legend.hist = TRUE,
    legend.is.portrait = TRUE,
    legend.hist.z = 0.05,            # reduce histogram size
    legend.title.size = 0.9,         # reduce title size
    legend.text.size = 0.2           # reduce text size
  ) +
  tm_layout(
    legend.outside = TRUE,
    legend.height = 0.45,
    legend.width = 2.5,              # reduce width from 5.0
